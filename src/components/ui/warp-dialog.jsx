...
export function WarpDialog({ open: openProp, onOpenChange: setOpenProp, ...props }) {
...
export function WarpDialogTrigger({ asChild = false, ...props }) {
  ...
  return (
    <>
      <motion.div
        ...
        initial={{ scale: 0, opacity: 1, backgroundColor: "hsl(10, 64%, 77%)" }}
        animate={{
          scale: 10, opacity: 0.2, backgroundColor: "hsl(6, 93%, 56%)",
          transition: { duration: enterDuration, opacity: { duration: enterDuration, ease: "easeInOut" } },
        }}
        exit={{ scale: 0, opacity: 1, backgroundColor: "hsl(10, 64%, 77%)", transition: { duration: exitDuration } }}
      />
      <motion.div
        className="absolute blur-[100px] rounded-full bg-red-500/90 h-full w-full top-[-25%] left-[-50%]"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.9, scale: [1, 0.7, 1], transition: { duration: enterDuration } }}
...
function WarpDialogOverlay({ className, ...props }) {
  return (
    <div className={cn("fixed inset-0 z-50 bg-red-500/10", className)} {...props}>
      <WarpAnimations />
    </div>
  )
}

export function WarpDialogContent({ children, className, ...props }) {
